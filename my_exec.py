...
class MyExecCommand(sublime_plugin.WindowCommand, ProcessListener):
    def run(self, cmd = [], file_regex = "", line_regex = "", working_dir = "",flip=False,
    ...
    def finish(self, proc):
        ...
        if self.flip:
            ...
            try:
                edit = newFile.begin_edit()
                newFile.insert(edit, 0, self.output_view.substr(sublime.Region(0,self.output_view.size())))
                newFile.erase(edit, newFile.find('\[Finished in \d+\.\d+s\]', 0))
                newFile.set_syntax_file('Packages/CSS/CSS.tmLanguage')
            finally:
